...
class Engine:
    def __init__(self, N_particles, N_engines, Size, Temperature, dt, time_steps):
        ...
        self.R = np.zeros(
            (self.ts, self.N, 3)
        )  # array with particle_positions for every timestep. (timestep, particle, dimension)
        self.V = np.zeros_like(self.R)  # array with velocities, simmilar to self.R

        self.R[0] = (
            np.random.uniform(0, self.S, (self.N, 3)) - self.S / 2
        )  # place particles centred uniformly in first timestep  in 3 dimensions
        self.V[0] = np.random.normal(
            0, np.sqrt(self.k_B * self.T / self.m), (self.N, 3)
        )
    ...
    def simulate(self):
        ...
        for i in range(1, self.ts):  # for every timestep
            ...
            self.R[i][ps] = [
                *np.random.uniform(-self.S / 2, self.S / 2, (2)),
                self.S / 2,
            ]  # move particle to top
            self.V[i][ps] = np.random.normal(
                0, np.sqrt(self.k_B * self.T / self.m), (3)
            )
            # self.V[i][ps] = function for maxwell_boltzmann dist

            self.V[i] = np.where(
                abs(self.R[i]) < self.S / 2, self.V[i], -self.V[i]
            )  # if particle at wall, flip speed
