...
class Rocket(Rocket):
    def begin_interplanetary_journey(
        self, system, mission, destination, k=10, verbose=True
    ):
        """
        ...
        for c, b in zip(coasts, boosts):
            if c[0] != 0:
                ...
                if coasted.status:
                    ...
                    print(
                        f"Actual coast time: {self.system.year_convert_to(coasted.t[-1] - coasted.t[0], 'L')}"
                    )
                    break
            self.boost(b)
    ...
    def coast(self, time, dt=1e-5, stop=True):
        ...
        def dominant_gravity(t, u):
            ...
            return Rnorm[self.dest + 1] - np.linalg.norm(r) * np.sqrt(
                self.Masses[self.dest + 1] / (self.k * self.Masses[0])
            )
        ...
        N = int(
            round(nT / planets_pos.shape[-1])
        )  # number of new points pr point in planetpos
        nT = N * planets_pos.shape[-1]  # make nT and N exactly compatible
        Tlin = np.linspace(T0, T1, nT)

        self.ri = np.repeat(
            planets_pos, N, axis=2
        )  # have planetpos in as many points as times
        for i in range(planets_pos.shape[-1] - 1):
            k0 = i * N
            k1 = k0 + N
            self.ri[:, :, k0:k1] = np.transpose(
                np.linspace(self.ri[:, :, k0], self.ri[:, :, k1], N), (1, 2, 0)
            )
    ...
    def leave_orbit_boost(self):
        ...
        v = np.sqrt(
            2 * const.G_sol * self.system.star_mass * (1 / np.linalg.norm(pos))
            - 1 / self.system.semi_major_axes[self.dest]
        )
    ...
    def enter_stable_orbit_boost(self):
        planet_pos = self.system.d_pos[
            :, self.dest, np.argmin(abs(self.system.time - self.travel_time))
        ]
        ...
        Vp = np.sqrt(
            const.G_sol * self.system.star_mass * (2 / planet_pos_norm - 1 / a)
        )
...
if __name__ == "__main__":
    ...
    cmds = [0, dv1, tH, dv2]  # Perform a Hohmantransfer
    ...
    Volcano.begin_interplanetary_journey(system, mission, destination=destination, k=10)
    ...
    t, r, v = travel.orient()
    travel.boost(v * dv1 / np.linalg.norm(v))
    travel.coast(tH)
    t, r, v = travel.orient()
    travel.boost(v * dv2)

    travel.record_destination(destination)

    plt.scatter(*r)
    plt.show()

    """
    We were unable to succesfully travel between the planets,
    and ended up using a shortcut. 
    """
