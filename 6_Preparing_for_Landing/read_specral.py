...
class Chi_square:
    # Very similar to least_square in data_analysis. Same thing, different function
    ...
    def f(self, L, Fmin, S, L0):  # analytic formula
    ...
    def residual(self, m, betas):
        ...
        L0 = betas[:, :, :, 2]
        return (
            (self.fluxes[m] - self.f(self.lambdas[m], Fmin, S, L0)) / self.sigmas[m]
        ) ** 2
...
class Atmosphere:
    ...
    def find_l_interval(self, lz, mxD=3e4):
        Dmax = lz / mxD
        L_min = lz - Dmax
        L_max = lz + Dmax
    ...
    def analyse_data(self, res=50, mxD=3e4):
        ...
        for mass, lz, i in zip(self.Ms, self.LZs, np.arange(self.N)):
            ...
            analysis = molecyl.find_best(
                0.7, 1, *self.Sigma(lz, mass * const.m_p), L_min, L_max
            )
    ...
    def plot(self):
        for name, lz, i in zip(self.Ns, self.LZs, np.arange(self.N)):
            ...
            plt.title(f"{name}, lambda0 = {lz}nm")
            plt.xlabel("Wavelength [nm]")
            plt.ylabel("Relative flux")
            plt.savefig(f"{name}_{lz}.png", bbox_inches="tight")
            plt.clf()
