...
class SolarSys(SolarSystem):
    ...
    Helt lik klassen i two_body_system, men generalisert for N planeter
    """
    ...
    def N_body_system(self, yrs, dt_pr_yr):
        def A(r):
            # Gravity only workig between star and planets, not among planets
            R_p = r[0] - r[1:]
            F_p = (  # F = GMmr / |r|^3
                -const.G_sol
                * R_p
            ...
            )  # transposes to make axis fit. Works because it works
            A_s = np.sum(F_p, axis=0) / self.star_mass  # acceleartion of star
            A_p = -F_p / np.transpose([self.my_masses] * 2)  # acceleartion of planets
    ...
    def radial_vel(self, i=np.pi):
        # Adds noise and pecular velocity to velocity of star
        ...
        plt.plot(self.time, self.nonoise)
        plt.show()
