...
# functions for stereographic projection
...
def xy_to_thph(x, y, th0=np.pi / 2, ph0=0):
    ...
    phi = ph0 + np.arctan(
        x * sin(beta) / (rho * sin(th0) * cos(beta) - y * cos(th0) * sin(beta))
    )
...
def generate_png(angles):
    ...
    for i in range(len(ang[0])):
        ...
        # fill RGB with values from himmelkule
...
if __name__ == "__main__":
    ...
    # take_360_pictures()
    # np.save("../verification_data/ang_ori_refs.npy.npy", generate_refernce_ids())

    reference_set = np.load("../verification_data/ang_ori_refs.npy")
    N = 1000
    fail = 0
